...
def procesar(pathname):
    ...
    for linea in datos:
        procesar_registro(instancia, linea)
    
    return True

def procesar_registro(instancia, linea):
    ...
    logger.debug("Comienzo a armar un nuevo objeto Venta")
    for campo in zip(campos, datos):
        logger.debug("%s -> %s" % (campo[0], campo[1]))
        objeto.importar_campo(campo[0].rstrip(), unicode(campo[1].rstrip(), 'iso-8859-1'))
