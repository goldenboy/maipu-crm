...
logger = logging.getLogger("accesorio_historico")
...
def procesar(instancia, pathname):
    ...
    for linea in datos:
        ...
        if(not ret):
            ...
            if(not hubo_errores):
                errores = open('accesorio_historico-' + err_filename, 'w')
            errores.write(linea)
            errores.flush()
            hubo_errores = True
...
def procesar_linea(instancia, linea):
    ...
    # Para cada accesorio a importar, hago una busqueda de accesorios por 
    # numero_factura, por si los accesorios fueron ingresados previamente
    busq = instancia.modulos['mm002_Accesorios'].buscar(numero_orden=datos[0])
    if len(busq) != 0:
        # si hay algun resultado, uso el primero
        objeto = busq[0]
    else:
        # Creo un objeto nuevo del modulo Turnos.
        objeto = sugar.ObjetoSugar(instancia.modulos['mm002_Accesorios'])

    # Defino la plantilla con los campos.
    campos = ['numero_orden', 'cliente_id', 'fecha_facturacion', 'marca_codigo',
            'accesorio_descripcion_c', 'importe', 'accesorio_tipo_c']
    ...
    # Voy a darle un valor al campo 'name', utilizando la descripcion del acc.
    logger.debug("Dando nombre al accesorio.")
    nombre = objeto.obtener_campo('accesorio_descripcion_c').a_sugar()
    objeto.importar_campo('name', unicode(nombre))



    # Aqui ya estan creadas todas las entradas en Sugar de las cuales este
    # accesorio depende. Ya puedo agrear el accesorio a la base de datos.

    objeto.importar_campo('currency_id', currency_id)
    logger.debug("Objeto listo.")

    logger.debug("Grabando un nuevo ACCESORIO...")
...
def obtener_instancia():
    ...
    instancia = sugar.InstanciaSugar(crm_config.WSDL_URL, crm_config.USUARIO,
                    crm_config.CLAVE, ['mm002_Accesorios', 'mm002_Marcas',
                                        'Contacts'],
